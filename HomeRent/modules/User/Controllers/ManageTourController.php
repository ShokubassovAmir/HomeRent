...
class ManageTourController extends FrontendController
{
    ...
    public function createTour(Request $request)
    {
        ...
        if (!empty($request->input())) {
            ...
            if ($row->save()) {
                ...
                $row->saveSEO($request);
                return redirect('user/apartment')->with('success', __('Апартамент создан'));
            }
        } else {
            $row = new Tour();
            $row->fill([
    ...
    public function editTour(Request $request, $id)
    {
        ...
        $row = $row->find($id);
        if (empty($row)) {
            return redirect('user/apartment')->with('warning', __('Апартамент не найден!'));
        }
        if (!empty($request->input())) {
            $request->validate([
                'title' => 'required',
            ...
            if ($row->save()) {
                $row->saveMeta($request);
                $row->saveSEO($request);
                return redirect('user/apartment')->with('success', __('Апартамент обновлен!'));
            }
        }
        $data = [
            'row'           => $row,
    ...
    public function deleteTour($id)
    {
        ...
        return redirect('user/apartment')->with('success', __('Апартамент удален!'));
    }
}