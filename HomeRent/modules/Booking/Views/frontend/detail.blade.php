            ...
            <div class="row booking-success-notice">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="notice-success">
                            <p class="line1"><span>{{$booking->first_name}},</span>
                                {{__('ваш заказ был успешно отправлен!')}}
                            </p>
                        <p class="line2">{{__('Детали бронирования отправлены на:')}} <span>{{$booking->email}}</span></p>
                        </div>
                ...
                <div class="col-lg-4 col-md-4">
                    <ul class="booking-info-detail">
                        <li><span>{{__('Номер заказа')}}:</span> {{$booking->id}}</li>
                        <li><span>{{__('Дата бронирования')}}:</span> {{display_date($booking->created_at)}}</li>
                        @if(!empty($gateway))
                        <li><span>{{__('Способ оплаты')}}:</span> {{$gateway->name}}</li>
                        @endif
                    </ul>
                </div>
            </div>
            <div class="row booking-success-detail">
                <div class="col-md-8">
                    ...
                    <div class="text-center">
                        <a href="{{url('user/booking-history')}}" class="btn btn-primary">{{__('История бронирования')}}</a>
                    </div>
                </div>
                <div class="col-md-4">
