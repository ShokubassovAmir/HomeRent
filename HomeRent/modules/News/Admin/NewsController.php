...
class NewsController extends AdminController
{
    ...
    public function create(Request $request)
    {
        ...
        if (!empty($request->input())) {
            ...
            if ($row->save()) {
                ...
                $row->saveSEO($request);
                return redirect('admin/module/news/create')->with('success', __('Новость добавлена!'));
            }
        }
        $data = [
    ...
    public function edit(Request $request, $id)
    {
        ...
        if (!empty($request->input())) {
            ...
            if ($row->save()) {
                $row->saveTag($request->input('tag_name'), $request->input('tag_ids'));
                $row->saveSEO($request);
                return redirect('admin/module/news/edit/' . $id)->with('success', __("Новости обновлены!"));
            }
        }
        $data = [
            'row'  => $row,
    ...
    public function bulkEdit(Request $request)
    {
        ...
        if (empty($ids) or !is_array($ids)) {
            return redirect()->back()->with('error', __('Не выбрано!'));
        }
        if (empty($action)) {
