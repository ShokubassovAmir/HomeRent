...
class TagController extends AdminController
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($request->input('name'))) {
            ...
            if (Tag::where('name', $request->input('name'))->exists()) {
                return redirect('admin/module/news/tag')->with('error', __('Тэг уже существует!'));
            } else {
                if ($row->save()) {
                    $row->saveSEO($request);
                    return redirect('admin/module/news/tag')->with('success', __('Тэг создан!'));
                }
            }
        }
    ...
    public function edit(Request $request, $id)
    {
        ...
        if (!empty($request->input())) {
            ...
            if ($row->save()) {
                $row->saveSEO($request);
                return redirect('admin/module/news/tag')->with('success', __('Тэг обновлен!'));
            }
        }
        $data = [
            'row'     => $row,
