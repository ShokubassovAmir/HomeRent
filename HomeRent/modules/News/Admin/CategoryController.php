...
class CategoryController extends AdminController
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($request->input('name'))) {
            ...
            if (NewsCategory::where('name', $request->input('name'))->where('parent_id',$request->input('parent_id'))->exists()) {
                return redirect()->back()->with('error', __('Категория уже существует!'));
            } else {
                $row->status = 'publish';
                $row->parent_id = $request->input('parent_id');
                if ($row->save()) {
                    $row->saveSEO($request);
                    return redirect('admin/module/news/category')->with('success', __('Категория создана!'));
                }
            }
        }
    ...
    public function edit(Request $request, $id)
    {
        ...
        if (!empty($request->input())) {
            ...
            if ($row->save()) {
                $row->saveSEO($request);
                return redirect('admin/module/news/category')->with('success', __('Категория обновлена'));
            }
        }
        $data = [
            'row'     => $row,
    ...
    public function bulkEdit(Request $request)
    {
        ...
        if (empty($ids) or !is_array($ids)) {
            return redirect()->back()->with('error', __('Выберите хотя бы 1!'));
        }
        if (empty($action)) {
            return redirect()->back()->with('error', __('Выберите действие!'));
        }
        if ($action == 'delete') {
            foreach ($ids as $id) {
        ...
        return redirect()->back()->with('success', __('Успешно обновлено!'));
    }
}
