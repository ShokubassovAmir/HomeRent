...
@section ('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{__('Все заказы')}}</h1>
        </div>
        ...
        <div class="filter-div d-flex justify-content-between">
                        ...
                        <select name="action" class="form-control">
                            ...
                            @if(!empty($statues))
                                @foreach($statues as $status)
                                    <option value="{{$status}}">{{__(':name',['name'=>ucfirst($status)])}}</option>
                                @endforeach
                            @endif
                            <option value="delete">{{__("Удалить заказ")}}</option>
                        </select>
            ...
            <div class="col-left">
                <form method="get" action="" class="filter-form filter-form-right d-flex justify-content-end">
                    ...
                    <button class="btn-info btn btn-icon" type="submit">{{__('Поиск')}}</button>
                </form>
            </div>
        </div>
                ...
                <form action="" class="bravo-form-item">
                    <table class="table table-hover bravo-list-item">
                        ...
                        <tr>
                            ...
                            <th>{{__('Обслуживание')}}</th>
                            <th>{{__('Клиент')}}</th>

                            <th>{{__('Сумма')}}</th>
                            <th width="80px">{{__('Статус')}}</th>
                            <th width="150px">{{__('Способ оплаты')}}</th>
                            <th width="120px">{{__('Создано')}}</th>
                            <th width="80px">{{__('Действия')}}</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($rows as $row)
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($service = $row->service)
                                        <div class="dropdown">
                                            <button class="btn btn-secondary btn-sm dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{__('Действия')}}
                                            </button>
                                            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                                                <a class="dropdown-item" href="#" data-toggle="modal" data-target="#modal-booking-{{$row->id}}">{{__('Деталь')}}</a>
                                                <a class="dropdown-item" href="{{url('admin/module/report/booking/email_preview/'.$row->id)}}">{{__('Просмотр')}}</a>
                                            </div>
                                        </div>
