<div class="b-panel-title">{{__('Информация о бронировании')}}</div>
<div class="b-table-wrap">
    <table class="b-table" cellspacing="0" cellpadding="0">
        <tr>
            <td class="label">{{__('Номер заказа')}}</td>
            <td class="val">#{{$booking->id}}</td>
        </tr>
        <tr>
            <td class="label">{{__('Статус бронирования')}}</td>
            <td class="val">{{$booking->statusName}}</td>
        </tr>
        @if($booking->gatewayObj)
            <tr>
                <td class="label">{{__('Способ оплаты')}}</td>
                <td class="val">{{$booking->gatewayObj->getOption('name')}}</td>
            </tr>
        @endif
        <tr>
            <td class="label">{{__('Название апартамента')}}</td>
            <td class="val">
        ...
        @if($booking->start_date && $booking->end_date)
            <tr>
                <td class="label">{{__('Дата начала')}}</td>
        ...
        <tr>
            <td class="label fsz21">{{__('Итого')}}</td>
            <td class="val fsz21"><strong style="color: #FA5636">{{format_money($booking->total)}}</strong></td>
        </tr>
    </table>
</div>
<div class="text-center mt20">
    <a href="{{url('user/booking-history')}}" target="_blank" class="btn btn-primary">{{__('Управление бронированием')}}</a>
</div>
