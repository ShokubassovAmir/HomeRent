...
class PageController extends AdminController
{
    ...
    public function create(Request $request)
    {
        if (!empty($request->input())) {
            ...
            if ($row->save()) {
                $row->saveSEO($request);
                return redirect('admin/module/page')->with('success', __('Страница создана'));
            }
        } else {
            $row = new Page();
            $row->fill([
    ...
    public function edit(Request $request, $id)
    {
        ...
        if (!empty($request->input())) {
            ...
            if ($row->save()) {
                $row->saveSEO($request);
                return redirect('admin/module/page')->with('success', __('Страница обновлена!'));
            }
        }
        $data = [
            'row'         => $row,
    ...
    public function bulkEdit(Request $request)
    {
        ...
        if (empty($action)) {
            return redirect()->back()->with('error', __('Действие не выбрано!'));
        }
        if ($action == "delete") {
            foreach ($ids as $id) {
