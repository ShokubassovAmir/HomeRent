<form class="bravo-form-login" method="POST" action="{{ route('login') }}">
    ...
    <div class="form-group">
        <input type="text" class="form-control" name="email" autocomplete="off" placeholder="{{__('Адрес электронной почты')}}">
        <i class="input-icon fa fa-envelope-o"></i>
        <span class="invalid-feedback error error-email"></span>
    </div>
    <div class="form-group">
        <input type="password" class="form-control" name="password" autocomplete="off"  placeholder="{{__('Пароль')}}">
    ...
    <div class="form-group">
        <div class="d-flex justify-content-between">
            <label for="remember-me" class="mb0">
                <input type="checkbox" name="remember" id="remember-me" value="1"> {{__('Запомнить')}} <span class="checkmark fcheckbox"></span>
            </label>
            <a href="{{ url("/password/reset") }}">{{__('Забыли пароль?')}}</a>
        </div>
    </div>
    ...
    @endif
    <div class="c-grey font-medium f14 text-center"> {{__('Впервые пользуетесь HomeRent?')}} <a href="" data-target="#register" data-toggle="modal">{{__('Зарегистрироваться')}}</a>
    </div>
</form>