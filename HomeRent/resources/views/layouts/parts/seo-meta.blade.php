@if(!empty($seo_meta))
    ...
    @endif
    <title>{{ $seo_meta['seo_title'] ?? $seo_meta['service_title'] ?? $page_title ?? ""}} - {{setting_item('site_title' ,'Booking')}}</title>
...
@else
    <title>{{ $page_title ?? ''}} {{setting_item('site_title' ,'Booking')}}</title>
